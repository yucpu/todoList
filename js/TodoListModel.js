...
class TodoListModel {
	 ...
	 */
	setEditItem(index){
		this.editItem = this.listToEdit.items[index]
	}
	/**
	 * 
	 * @author Yuchen Pu
	 ...
	 */
	moveItemUp(index){
		let editItem = this.listToEdit.items[index]; // index of selected item
		if(index > 0){ // if item was not first item in the lists, do follow operations
			let previousItem = this.listToEdit.items[index-1];  // store [index-1] into pre
			this.listToEdit.items[index-1]= editItem; // [index-1] = item
			this.listToEdit.items[index] = previousItem; //[index] = pre
		}else{
	...
	moveItemDown(index){
		let editItem = this.listToEdit.items[index]; 
		if(index < window.todo.model.listToEdit.items.length - 1){ 
			let laterItem = this.listToEdit.items[index - (-1)];  
			this.listToEdit.items[index - (-1)]= editItem; // 
			this.listToEdit.items[index] = laterItem;
		}else{
	...
	deleteItem(index){
		window.todo.model.listToEdit.removeItem(this.listToEdit.items[index]);
		this.view.loadItems(this.listToEdit);
     ...
     */
    goHome() {
		if (document.getElementById("list_name_textfield").value == ""){
			alert("You must input at least one character in the textfield!!");
			return;
		}
		let tempName = document.getElementById("list_name_textfield").value;
		for(let i=1;i<this.todoLists.length;i++){
			if(tempName == this.todoLists[i].getName()){
				alert("You can't use existed list's name!! ");
				return;
			}
		}
    ...
    goList() {
		...
		document.getElementById("item_due_date_picker").value = "null";
		this.editItem = null;
		if (this.listToEdit.getOwner() !== "Unknown"){
			document.getElementById("list_owner_textfield").value = this.listToEdit.getOwner();
		}else{
    ...
    compare(item1, item2) {
        ...
        // SORT BY COMPLETED
        else if(thisModel.isCurrentItemSortCriteria(ItemSortCriteria.SORT_BY_STATUS_DECREASING)
            || thisModel.isCurrentItemSortCriteria(ItemSortCriteria.SORT_BY_STATUS_INCREASING)){
